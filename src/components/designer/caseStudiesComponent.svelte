<script>
  ...
  import CaseStudiesComponent from "../designer/caseStudiesImgComponent.svelte" 
...
<svelte:head>
    <title>Case Studies</title>
     ...
     <div class="case-studies-container">
        {#each case_studies as item}
        <div class="case-studies-block">
            <div class="text-item">
                <h2>{item.title}</h2>
                <p>{item.text}</p>
                {#if item.role}<p class="role-item"><strong style="font-family: 'HelveticaNeue', Arial, Helvetica, sans-serif; font-weight: 700;">{item.role}</strong></p>{/if}
            ...
            <CaseStudiesComponent item={item}/>
        </div>
        {/each}
     </div>
